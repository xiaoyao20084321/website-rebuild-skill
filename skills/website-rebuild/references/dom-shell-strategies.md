...
- 结果**写回 `style.*`**（`transform: scale(x)`、`height`、`max-height`、`width`），而不是切一个类名。
...
- **用块自己的公式在页内对活矩形重算，再与页面产物比**——期望值是"公式作用在**这一刻的活盒子**上的结果"，不是一个冻结的数字。冻结数字换个视口就全线红，而且会掩盖真错（同 `verification-gates.md` §4.4：断机制本身，不断某次录制里它恰好等于多少）；
...
- **记录纪律**：过渡 / 动画途中量到的高度**不进比对产物**——它属于"这一次运行"，记派生判定（`verification-gates.md` §4.10）。
